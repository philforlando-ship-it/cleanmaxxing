// POST /api/plan/pattern-d/trt/start-protocol
// User started TRT. Creates one intervention row with type='trt' +
// status='on_protocol' + the optional protocol details. Idempotent:
// re-clicking the start button when an active row already exists
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('trt_intervention_create_failed', err);
    return NextResponse.json(
