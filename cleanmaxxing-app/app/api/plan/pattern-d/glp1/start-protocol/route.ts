// POST /api/plan/pattern-d/glp1/start-protocol
// User started a GLP-1 protocol. Creates one intervention row with
// type='glp1' + status='on_protocol' + the optional protocol details.
// Idempotent: re-clicking the start button when an active row already
// exists returns the existing id rather than creating a duplicate.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('glp1_intervention_create_failed', err);
    return NextResponse.json(
