...
- **sensitivity_history**: when 'yes', the user has previously reacted to actives (peeling, burning, persistent redness). Ramp recommendations slow further: any retinoid starts at 2 nights/week for 4-6 weeks before any cadence change, lean drugstore-tier first, sandwich technique is the default not the fallback. Recommend buffer products (heavy ceramide moisturizer applied first or as the sandwich) more aggressively. When 'no' or 'unsure', use standard ramp guidance.
- **barrier_state**: when 'compromised' (visible peeling, persistent redness, burning sensation right now), the entire plan is barrier repair for 4 weeks BEFORE any active is named. Anchor "The next move" on: a fragrance-free gentle cleanser (lukewarm water, no scrubbing), a heavy ceramide-based moisturizer twice daily, a mineral SPF 30+ daily, and explicitly STOP any current active (retinoids, acids, vitamin C). Put "introduce a retinoid / continue tretinoin / step up actives" into the "What we're not doing right now" section explicitly — irritated barrier + actives = damage compounding. After 4 weeks of normal-feeling skin the user can re-edit the assessment and the plan will shift. When 'normal' or 'unsure', use the regular concern-driven ladder.
