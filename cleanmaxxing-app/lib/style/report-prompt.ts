...
- **facial-structure × style coordination (2026-05-11)** — when facial_structure_chin_jaw_concern or facial_structure_face_first_distribution is set, the wardrobe is framing a face the user has named specific reads on. Use the signals to bias collar / V-neck / accessory choices; do NOT recite the concerns back at the user.
  - **chin_jaw_concern includes 'submental_fullness' or 'chin_neck_transition'**: avoid high crew necks and turtlenecks (they amputate the neck line and amplify the read the user already named). Bias toward open collars, V-necks, henleys, and one-button-undone shirt collars to lengthen the visible neck. For outerwear, prefer notch lapels and structured collars over crewneck sweatshirts and high-zip technical jackets in close-to-face layers.
  - **chin_jaw_concern includes 'overall_softness' or 'jaw_definition_front'**: structured face-frame pieces do more here. Stronger acetate eyewear / sunglasses, structured outerwear collars, and a deliberate beard or stubble (deferred to facial_hair plan, but worth a single mention) all add definition the face isn't carrying alone. Avoid soft round-collar pieces in the close-to-face zone.
  - **chin_jaw_concern includes 'chin_projection_side'**: side-profile is the user's specific concern. Hat brim choice matters — flat brim pulls attention forward and competes with weak chin projection; curved brim or no-hat reads better. Beard cadence (deferred to facial_hair) is a real lever; the wardrobe can support it via collar and outerwear structure.
  - **face_first_distribution = 'face_softer_than_body'**: the wardrobe is framing a face that holds softness even when the body leans out. Push toward structured collars, more architectural eyewear, and avoid overly soft fabrics (loose drape knits, slouchy collarless tops) in the close-to-face zone. Same architecture push as the balding case, even when balding signal is mild.
  - **face_first_distribution = 'face_sharper_than_body'**: the face is doing the framing for free. Standard prescription stands — no special structural push needed. If anything, lean into the user's high-feasibility envelope (open collars, soft fabrics, casual eyewear all work because the face anchors them).

  IMPORTANT — fire only when the relevant components are actually being recommended in "The next move" or "This week." This is per-component guidance, not a section to insert. Do NOT recite "your chin_jaw_concern is X" back at the user.
