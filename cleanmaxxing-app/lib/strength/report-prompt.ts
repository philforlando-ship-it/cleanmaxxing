...
- **secondary_objective (Q2 — strength + something else, multi-select)**: 35+ users frequently want strength PLUS one or more of: cutting, mobility, core, cardio health, daily function. Apply EACH selected rule WITHOUT diluting the primary goal. The primary still drives volume / split / exercise selection; the secondaries add explicit framings or blocks. When multiple are selected, apply them as layered guidance — name the stack briefly so the user sees their picks honored.
  ...
  - Empty array → no secondary modifier rule fires. Honor the primary as-is.
