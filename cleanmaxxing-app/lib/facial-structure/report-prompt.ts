...
- Buccal fat removal under 30: the cosmetic procedure POV (28) flags this as the highest-regret procedure for young, lean, narrow-faced candidates. If age < 30 AND cosmetic_procedure_openness is 'actively_considering' AND chin_jaw_concern *includes* 'submental_fullness' or 'overall_softness', explicitly call this out.
...
- Else if BF is sub-15 AND posture is clean AND puff is rarely AND chin_jaw_concern *includes* 'chin_projection_side' or 'jaw_definition_front' (i.e., a genuine structural deficit is among the selections) AND cosmetic_procedure_openness is 'curious_about_options' or 'actively_considering' → the lever is the cosmetic Pattern D shell. Direct them to /plan/procedures for the filler-as-diagnostic → implant pathway (POV 28). Do NOT recommend a specific procedure here.
...
- **chin_jaw_concern** (multi-select array; 'no_specific_concern' is exclusive and means no diagnostic anchor) — chin_projection_side routes to the side-profile lever stack (chin filler / implant if Pattern D unlocks). jaw_definition_front routes to BF + masseter + framing. chin_neck_transition routes to neck training + posture. submental_fullness in a lean user = puff diagnostic; in a non-lean user = cut. overall_softness almost always = cut as primary lever. When multiple concerns are selected, lead with the one whose lever overlaps the user's other modifiers (e.g., select 'chin_neck_transition' first when postural_pattern includes 'forward_head'; select 'jaw_definition_front' first when BF is over 15). When 'no_specific_concern' is the only selection, lead with framing — the user is here for general direction, not a targeted fix.
